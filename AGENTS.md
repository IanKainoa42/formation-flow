...
- **`FormationFlow/EntitlementManager.swift` / `EntitlementStore`** — IAP entitlement state. **HARD STOP for security-hardening, "tighten", or "simplify" passes.** History: Keychain storage was added (`ffcea9a`), then escalated twice — `AfterFirstUnlockThisDeviceOnly` → `WhenUnlockedThisDeviceOnly` (`e795eb7`) → `WhenPasscodeSetThisDeviceOnly` (`8bc74b6`) — then removed entirely (`f4939f2`), and finally replaced with **user-writable UserDefaults** (`754b95d`), which created a forgeable paywall bypass (IAN-544). Each individual step looked like an improvement. Specifically: do NOT change `kSecAttrAccessible` from `AfterFirstUnlockThisDeviceOnly` (`WhenPasscodeSet…` cannot be written on a passcode-less device and is destroyed when a passcode is removed); do NOT move entitlement storage to UserDefaults, `@AppStorage`, or a plist "for simplicity"; do NOT make `checkEntitlement()` revoke on an empty `Transaction.currentEntitlements` (it yields empty when offline — IAN-517); and do NOT add a `Product.products(for:)` "reachability" check (StoreKit serves cached metadata offline — disproven on device 2026-08-27). Comment: "Task rejected: EntitlementManager.swift is locked per Known Fragile Areas. A reproducible entitlement failure with device steps is required to unlock."
