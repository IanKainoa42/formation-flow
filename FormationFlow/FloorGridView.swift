...
struct FloorGridView: View {
    ...
    private func canvasStack(geometry: GeometryProxy) -> some View {
        let baseCellSize = min(geometry.size.width / CourtConstants.width, geometry.size.height / CourtConstants.height)
            ...
            .onChanged { value in
                if !isDraggingAthlete && !isPanning {
                    ...
                    for (index, athlete) in formation.athletes.enumerated() {
                        if PathCalculations.squaredDistance(from: startScaled, to: athlete.position) < CourtConstants.hitRadiusSquared {
