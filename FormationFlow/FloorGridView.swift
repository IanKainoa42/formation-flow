...
struct FloorGridView: View {
                    ...
                    .onChanged { value in
                        ...
                        if rotationStartPositions.isEmpty {
                            ...
                            rotationStartPositions = Dictionary(
                                uniqueKeysWithValues: renderedAthletes.compactMap {
                                    selectedAthleteIDs.contains($0.id) ? ($0.id, $0.position) : nil
                                }
                            )
                        }
    ...
    private func authenticateAndDeleteWaypoint() {
    ...
    private func authenticateAndResetAllPaths() {
        let context = LAContext()
        var error: NSError?

        if context.canEvaluatePolicy(.deviceOwnerAuthentication, error: &error) {
            let reason = "Authentication is required to perform sensitive destructive actions and securely reset paths."
            context.evaluatePolicy(.deviceOwnerAuthentication, localizedReason: reason) { success, authError in
                DispatchQueue.main.async {
                    if success {
                        self.resetAllPaths()
