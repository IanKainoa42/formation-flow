...
## 2026-05-23 — Correction: don't fix drag/hit-test bugs in the global render path

- **Category:** correction
- **What happened:** To fix a transition-preview drag bug (grabbing an athlete mid-scrub popped it + swapped formation appearance), I changed `renderedAthletes`/`displayProgress` in FloorGridView to default `focusedEndpoint ?? currentFormationEndpoint` and lock the canvas to start/end whenever paused. This compiled and "fixed" the grab, but broke two things the user immediately caught: (1) the progress scrubber no longer animated the floor (locked to 0/1), and (2) pausing mid-playback jumped athletes back to the section start. `renderedAthletes` is the SINGLE source for playback, scrub, pause AND hit-testing — changing it globally breaks the paused-interpolation states (scrub/pause) that legitimately need `player.currentAthletes`.
- **Rule:** The paused-but-interpolated states (scrubbing, pause-mid-play) require `focusedEndpoint == nil` so `renderedAthletes` returns `player.currentAthletes`. Never make `renderedAthletes`/`displayProgress` fall back to an endpoint when `focusedEndpoint` is nil — that's the signal that distinguishes "editing a formation" from "viewing an interpolated frame." Fix interaction bugs (drag pop, hit-test mismatch) in the DRAG HANDLER (capture `dragStartPositions` consistently with the visible frame, resolve `editableID`), not in the shared render computed property. Build success ≠ behavior correct — playback/scrub/pause must be manually exercised after any render-path edit.

## 2026-05-23 — Best practice: one transport control stack per orientation

- **Category:** best_practice
- **What happened:** iPad portrait showed TWO transport stacks when an athlete was selected — the selected-athlete inspector (`SelectedAthleteSidebarView` in AthleteDetailPanel.swift) embeds its own transport with no orientation gate, while the bottom overlay (`FormationHomeView` detail `.overlay(alignment: .bottom)`, gated `isIPadPortrait`) also renders. The empty-selection sidebar transport was already gated `!isIPadPortrait`; the inspector copy leaked.
- **Rule:** Exactly one transport stack visible at a time: portrait → bottom overlay only; landscape → sidebar/inspector only. iPad portrait vs landscape is NOT distinguishable by size class (both regular width) — use `isIPadPortrait` (GeometryReader width-vs-height in FormationHomeView) and thread it into any view that embeds a transport. Documented as "UI Layout Policy" in repo CLAUDE.md and logged to truth-service as a `decision`.

## 2026-05-23 — Best practice: animate auto-rewind, and owner approval unlocks Models.swift

- **Category:** best_practice
- **What happened:** The transition player's idle auto-rewind (`scheduleIdleReset` → after ~5s of pause it returned to start) did an instant `seek(to: 0)`, which looked like an unintentional pop. Ian asked to keep the hold but animate the rewind so it looks deliberate. Implemented an isolated `rewindTimer: AnimationTimer` + `animateRewindToStart()`/`rewindStep()` that drives `progress` 1→0 over ~0.5s; the canvas follows because `isTransportEngaged` (progress>0) keeps it tracking `currentAthletes`. `cancelRewind()` added to play/seek/reset to avoid fighting.
- **Rule:** For "auto-snap" UX (idle reset, return-to-start), prefer animating a driven value over an instant set — reuse the existing `AnimationTimer` 60fps pattern (`AnimationTimer { [weak self] in self?.step() }`), don't fold reverse logic into the forward `update()` loop. Models.swift is a HARD STOP file for AUTONOMOUS agents (Jules/Bolt/Palette), but a direct request from Ian (the owner) is explicit approval and overrides the freeze — instruction priority: user's explicit instructions are highest. When touching a locked file on owner request, keep the change surgical/isolated and record the approval in the learning/memory so future sessions know why the freeze was bypassed.

## 2026-05-23 — Simulator screenshot automation: no headless rotation; portrait dims are valid; seed via workspace.v1.json

- **Category:** knowledge_gap
- **What happened:** Ran the `.appstore/capture.md` screenshot flow. Three things bit me / were resolved: (1) the spec's seeding step (preload UserDefaults key `routine.v1`) is STALE — `Models.swift` `load()` now reads `Documents/workspace.v1.json` (a `RoutineWorkspace`); (2) `capture.md` wants iPad in LANDSCAPE, but there is no headless way to rotate a simulator — `xcrun simctl` has no orientation command (`simctl ui` only does appearance/contrast), and the GUI paths (`osascript` key code 124 / `peekaboo hotkey --keys "cmd,right" --app Simulator`) both failed in a plain Claude Code/Bash session: osascript hit "not allowed to send keystrokes" (1002, needs Accessibility) and peekaboo needs Screen Recording permission; (3) raw `simctl io <udid> screenshot` of iPad Pro 13" M5 returns 2064×2752 (portrait framebuffer).
- **Rule:** For the FormationFlow screenshot flow: (a) SEED by copying `.appstore/demo_routine.json` straight to `$(xcrun simctl get_app_container <UDID> com.ianrichardson.formationflow data)/Documents/workspace.v1.json` BEFORE launch — never UserDefaults. (b) iPad LANDSCAPE capture requires GUI-automation permissions (Screen Recording for peekaboo / Accessibility for osascript) — available to the 5:32am LaunchAgent (runs under Ian's user) but NOT to an interactive Claude Code shell. If you can't rotate, capture PORTRAIT: 2064×2752 is itself a VALID App Store iPad 13" portrait dimension, so the asset is usable as-is — don't `sips -r -90` a portrait-composed UI (that yields sideways content). (c) Only `iPad Pro 13-inch (M5)` and `iPad Pro 11-inch (M5)` sims are installed — no M4, no 12.9"; the 13" M5 covers the 13" slot exactly but there's no native 12.9" path. (d) `.appstore/demo_routine.json` is thin (3 athletes, 2 roles) — enrich in-app and re-export for richer shots.

## 2026-06-19 — Canvas perf: don't animate static overlays

- **Category:** best_practice
- **What happened:** Added per-athlete `sin()`-pulsing rings (destination spotlight + "didn't make it" warnings) to FloorCanvasView, and forced the TimelineView to 30fps whenever a single athlete was selected. With ~18 athletes flagging at once, the SwiftUI Canvas repainted the entire floor (grid + athletes + paths + ghosts + rings) 30×/sec continuously — bogged down the whole Mac under Catalyst.
- **Rule:** Only drive the canvas clock fast for things that genuinely move frame-to-frame (the comet at 30fps). Selection rings, reachability warnings, endpoint markers = STATIC, drawn once per redraw, never their own `sin()` pulse and never a reason to raise the tick rate. The step LED "persists" between beats, so it ticks at ~12fps (only changes on the beat snap), not 30. A pulsing overlay = a permanent full-canvas repaint loop.

## 2026-06-19 — Sheets need @EnvironmentObject re-injected
...
## 2026-06-21 — Catalyst onboarding visual-verification gotchas

- **Category:** best_practice
- **What happened:** Verifying onboarding screen fixes on Mac Catalyst hit three snags. (1) `peekaboo image --app FormationFlow` captured a STALE already-running Catalyst instance (showed old copy "01/04"), not my fresh build — `open "$APP"` just focused the running one. (2) Onboarding screens 2–5 advance only by paged-TabView SWIPE (their `cta` is nil; only screens 1 & 6 have buttons), so there's no button to click to reach screen 4/5. (3) `peekaboo click --coords X,Y --app <name>` treats coords as WINDOW-relative and rejects screen coords.
- **Rule:** To screenshot a fresh Catalyst build: `killall FormationFlow; open -n "$APP"` (force new instance). To land on a specific onboarding screen deterministically, temporarily set `@State private var page = N` in OnboardingView, build, screenshot, then revert. For clicks use window-relative coords with `--app` (or `--global-coords` for screen-space). For a guaranteed-visible path collision in a demo, route two athletes head-on (A:P→Q, B:Q→P, same timing) — they meet at the midpoint at t=0.5, distance 0, always flagged by `findPathCollisionIDs`.
